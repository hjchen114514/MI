...
>
> **Goal (updated):** Fine-tune three LoRA adapters (Human-FT, Doped-108-FT, Doped-36x3-FT) plus run the base model with no adapter. Four conditions total. All four are extracted and projected onto the same Human-FT latent axis. Phase 4 produces per-condition KDE plots and a cross-condition Layer K summary table.
>
> **~~Two~~ Two doped datasets:**
> - `doped_108`: all 108 human responses rewritten by AI (mild doping, same unique scenarios as human data)
> - `doped_36x3` *(primary)*: 12 stratified samples × 9 rewrites = 108 examples (strong doping, concentrated AI reasoning signal)
>
...
| `scripts/stratified_sample.py` | **DONE** — draws 12 stratified seeds from human.jsonl for doped_36x3 construction |
...
> ```bash
> mkdir -p data models/human_ft models/doped_108_ft models/doped_36x3_ft scripts src/utils
> mkdir -p results/activations/base results/activations/human_ft
> mkdir -p results/activations/doped_108 results/activations/doped_36x3
> mkdir -p results/labels results/projections
> mkdir -p results/figures/base results/figures/doped_108 results/figures/doped_36x3
> touch src/__init__.py src/utils/__init__.py
> cp DK/human.jsonl data/human.jsonl
...
> - [x] **phase2_extract.py** — now runs 4 conditions: `[base, human_ft, doped_108, doped_36x3]`; auto-skips conditions with empty/missing adapter dirs; crash-fast on None activations with descriptive error
...
> - [x] **scripts/stratified_sample.py** — new script; draws 12 stratified seeds from human.jsonl for manual doped_36x3 construction
...
> **UPDATE [2026-04-06] — Rename + activation extraction overhaul:**
>
> **doped_12x9 → doped_36x3**: renamed throughout. 36 seeds × 3 rewrites. Preserves human-like/AI-like distribution of human dataset.
>
> **Activation extraction paradigm shift:** Original method (last prompt token, first forward pass) produced identical vectors for all 100 sessions — zero variance, KDE crash. Fixed by running an extra forward pass on `[prompt + full response]` with `use_cache=False` and extracting the last generated token's residual stream across all 32 layers. This produces genuine session-level variation. `hooks.py` redesigned with `active` flag and `target_pos` — hooks are silent during `model.generate()`, fire only during the explicit extra pass.
>
> **Hard cutoff:** `results/cutoff.json` cutoff field set to 18. Youden's J data preserved.
>
> **Phase 4:** base excluded (deterministic prompt → constant projections). Mean_diff retained for reference only — Cohen's d is the authoritative Layer K metric.
>
> **PoC Layer K result:** Layer 0 for both doped conditions (Cohen's d: doped_108=2.74, doped_36x3 varies by run). Layer 0 has both the highest Cohen's d AND highest JSD — the separation is tight and clean, not just a large raw shift.
