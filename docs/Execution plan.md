...
> │   ├── doped_36x3_seeds.jsonl     # NEW: 12 stratified samples before rewriting
> │   └── doped_36x3.jsonl           # NEW: 36×3 stratified+rewritten (strong doping)
> ├── models/
> │   ├── human_ft/
...
> DATA_DOPED_12X9 = "data/doped_36x3.jsonl"
>
> # Paths — models
> MODEL_BASE = None                         # base model, no adapter
> MODEL_HUMAN_FT = "models/human_ft"
> MODEL_DOPED_108_FT = "models/doped_108_ft"
> MODEL_DOPED_12X9_FT = "models/doped_36x3_ft"
> ```
...
> python src/phase1_finetune.py --dataset data/doped_36x3.jsonl --output models/doped_36x3_ft
> ```
...
> Models list: `[base, human_ft, doped_108, doped_36x3]`. The script auto-skips any model whose adapter directory is empty (e.g. doped_36x3 before that data is ready). Run order and outputs are the same but replicated across all 4 conditions. The base model is loaded with `adapter_path=None`.
...
> Phase 3 now auto-detects which conditions have extracted activations and projects each one. Output files: `base.npy`, `doped_108.npy`, `doped_36x3.npy` (shape `(32, 100)` each), plus `human_ft_held_out.npy` (shape `(32, 20)`). Latent vector is still built from Human-FT training sessions only — unchanged.
...
> ```bash
> # Generate stratified seeds for doped_36x3 (then rewrite ×9 with Claude Sonnet 4.6)
...
> | Limitation | Impact | Mitigation |
> |-----------|--------|-----------|
> | doped_36x3 only contains 36 unique reasoning scenarios repeated 3× | Model may overfit to those 36 patterns rather than learning general AI-style reasoning | Acknowledged as intentional amplification; doped_108 provides a milder comparison |
> | doped_108 is AI-rewritten from all 108 human responses without explicit AI reasoning injection | May not produce meaningfully different internal representations from human_ft | Serves as mild-doping baseline; doped_36x3 is the primary experimental condition |
...
> **UPDATE [2026-04-06] — Renamed doped_12x9 → doped_36x3; activation extraction method overhauled:**
>
> **Rename:** The stratified doped condition is renamed from doped_12x9 to **doped_36x3** (36 seeds × 3 rewrites). All file paths, model dirs, config constants, and code updated. The 36×3 design preserves the same human-like/AI-like split as the full human dataset.
>
> **Critical bug fixed — activation extraction:** The original Phase 2 extracted residual stream at the last prompt token during the first forward pass of `model.generate()`. Since the prompt is identical for all 100 sessions and the forward pass is deterministic, all 100 sessions produced identical activation vectors → zero variance → KDE crashed. Fix: after generation, run one extra forward pass on `[prompt + full response]` with `use_cache=False`, extract residual stream at the last generated token across all 32 layers. This produces genuine session-level variation because each session generates a different response. Output shape unchanged: `(100, 960)` per layer file.
>
> **Cutoff:** Youden's J artifact (cutoff=19) replaced with hard cutoff=18 (Arad & Rubinstein 2012). Only the active cutoff field in `results/cutoff.json` was changed; Youden's J data preserved.
>
> **Phase 4:** base model excluded from comparison (deterministic prompt → constant projections). Only doped_108 and doped_36x3 compared against human_ft.
>
> **Layer K results:**
>
> *doped_12x9 run (2026-04-05) — response dist: human_ft 40 human-like, doped_108 39, doped_12x9 59:*
> | Condition | Layer K | Cohen's d | JSD |
> |---|---|---|---|
> | doped_108 | 0 | 2.74 | 0.479 |
> | doped_12x9 | 0 | 3.35 | 0.585 |
>
> *doped_36x3 run (2026-04-06) — response dist: human_ft 33 human-like, doped_108 38, doped_36x3 40:*
> | Condition | Layer K | Cohen's d | JSD |
> |---|---|---|---|
> | doped_108 | 21 | 2.56 | 0.408 |
> | doped_36x3 | 21 | 2.22 | 0.419 |
>
> Layer K shifted 0 → 21 between runs. doped_36x3 (36 diverse seeds) produces less concentrated output than doped_12x9 (12 seeds) and shows weaker corruption signal. human_ft itself varies run to run (stochastic fine-tuning). Mean_diff retained for reference only — Cohen's d is the authoritative Layer K metric.
